...
void AdvancedText::DiscardDeviceResources()
{
    //SafeRelease(&pRT_);
    //SafeRelease(&pBlackBrush_);
}
...
HRESULT AdvancedText::DrawD2DContent()
{
    HRESULT hr;
...
void AdvancedText::OnResize(UINT width, UINT height)
{
    if (pRT_)
    {
        D2D1_SIZE_U size;
...
LRESULT CALLBACK AdvancedText::WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    if (message == WM_CREATE)
    {
        LPCREATESTRUCT pcs = (LPCREATESTRUCT)lParam;
        AdvancedText *pAdvancedText = (AdvancedText *)pcs->lpCreateParams;

        ::SetWindowLongPtrW(
            hwnd,
            GWLP_USERDATA,
            PtrToUlong(pAdvancedText));

        return 1;
    }

    AdvancedText *pAdvancedText = reinterpret_cast<AdvancedText *>(
        ...
        case WM_SIZE:
            {
                ...
                pAdvancedText->OnResize(width, height);
            }
            return 0;
