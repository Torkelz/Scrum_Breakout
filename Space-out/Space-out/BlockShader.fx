...
void GS( point VSOutput p_input[1], inout TriangleStream<GSOutput> p_outputStream)
{
	...
	float4 vert0 = p_input[0].m_posH + mul( float4(dimensions * float4(-1.0, -1.0, -1.0, 0.0)), m_WVP); // 0 ---
	float4 vert1 = p_input[0].m_posH + mul( float4(dimensions * float4( 1.0, -1.0, -1.0, 0.0)), m_WVP); // 1 +--
	float4 vert2 = p_input[0].m_posH + mul( float4(dimensions * float4(-1.0,  1.0, -1.0, 0.0)), m_WVP); // 2 -+-
	float4 vert3 = p_input[0].m_posH + mul( float4(dimensions * float4( 1.0,  1.0, -1.0, 0.0)), m_WVP); // 3 ++-
	float4 vert4 = p_input[0].m_posH + mul( float4(dimensions * float4(-1.0, -1.0,  1.0, 0.0)), m_WVP); // 4 --+
	float4 vert5 = p_input[0].m_posH + mul( float4(dimensions * float4( 1.0, -1.0,  1.0, 0.0)), m_WVP); // 5 +-+
